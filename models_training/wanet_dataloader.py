...
import random
import numpy as np
from model_lib.chest_cnn_model import data_loader
...
def get_WaNetdataloader(opt, train=True, pretensor_transform=False):
    transform = get_transform(opt, train, pretensor_transform)
    if opt.dataset == "gtsrb":
        dataset =  torchvision.datasets.GTSRB(root='./raw_data/', split='train', download=True, transform=transform)
    elif opt.dataset == "mnist":
        ...
        chest = data_loader(root_dir='raw_data\chest')
